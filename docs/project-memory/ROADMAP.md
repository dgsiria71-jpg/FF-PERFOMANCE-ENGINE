...
**Remaining item3 order:** (1) obtain genuinely adapter-owned scene/mode/load/environment evidence covering both full windows or leave unsupported Unknown and generic actions OFF; foreground/PresentMon FPS ≠ scene. (2) Investigate external benchmark/other mutation evidence with truthful coverage/blind spots. (3) Host owns state→classifier→eligibility→trusted action catalog→budget→reversible executor and every active lease, restores retained leases before session Reset/rebind, excludes concurrent Track0 benchmarks and suspends/reconciles Guardian. Generation observation is NOT exclusion; final-read and post-final-process-check races remain. Windows/real-game HIL before enabling. Item4 learned reliability and item5 post-session queue follow.
